...
| 4 | Agent call forwarding pattern | canopyd receives user message from frontend, wraps it with context manifest (from context compiler), sends `POST /v1/responses` to Hermes, streams SSE response back | The context compiler (which lives in canopyd — it traverses the DAG from ~~DuckDB~~ **PostgreSQL** (**superseded 2026-09-17** — DuckDB is ARCHIVED, see `specs/ARCHITECTURE.md` §3.2)) pre-assembles the token-budgeted context manifest. The manifest is sent as part of the Hermes API request body, not as a separate endpoint. |
...
Forwards a user message to the Hermes gateway. canopyd assembles the context manifest from ~~DuckDB~~ **PostgreSQL** (**superseded 2026-09-17** — DuckDB is ARCHIVED), maps the active profile, and calls `HermesClient.SendMessage()`.
...
1. **Build context compiler** — traverses ~~DuckDB~~ **PostgreSQL** graph (**superseded 2026-09-17** — DuckDB is ARCHIVED), produces `ContextManifest` with token budget enforcement, recency ordering, and profile injection
