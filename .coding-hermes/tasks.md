...
| CI-002 | CI runs stopped triggering: T208 push 29f1905 received by GitHub (PushEvent 2026-08-06T19:37:33Z), zero workflow runs created (build.yml active, on:push master intact, repo Actions enabled). Suspected org-level Actions block (billing). Verify: any push creates a run; human fix (Azure sub / per-user paid) per github-actions-billing-2026. **Probe history: T208 29f1905 → 0 runs; T209 d040cd0 → 0 runs; T210 3db4b19 → 0 runs; T211 22e708d → 0 runs (PushEvents recorded, zero workflow runs — 4th consecutive). Org-level block CONFIRMED persistent, not transient. ESCALATED to Bane Tick 210 (human fix: Azure sub / per-user paid). Probe #5 = T212 push. Close condition: a push triggers a run.** | High | 1 | — | — | ADMIN — org-level (Bane) | — | — |
