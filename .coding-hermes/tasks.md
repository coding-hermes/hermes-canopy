...
|| 🔴 BUG-021 | Config mismatch: canopyd uses DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME env vars, but CANOPY_DB_URL is documented. Setting CANOPY_DB_URL is silently ignored — server connects to wrong DB (localhost:5432 default vs expected 5437). Fix: Config.FromEnv() should accept CANOPY_DB_URL as override, or self-host guide must be corrected. | Critical | 2 | config | ++configuration, ++devops | | | |
|| 🔴 BUG-022 | Integration test FK violation: 8 handler tests + full db/ handler suite fail with `tree_members_user_id_fkey` — test helpers (createTreeViaHTTP, NewIntegrationPool) don't seed a sentinel user matching the test JWT before creating trees. Every PG-dependent integration test is broken. Fix: auto-seed user in NewIntegrationPool or createTreeViaHTTP helper. | Critical | 2 | testutil, handler | ++testing, ++db | | | |
| **Continuous** | | | | | | | | |
