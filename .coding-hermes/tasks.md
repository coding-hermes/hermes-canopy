...
| TEST-004 | PG test architecture: 224 tests × fresh DB + 21 migrations each (~5-20s setup/test) = db package ~8min, handler ~15min. NOT a hang — cumulative setup cost. Suites PASS with generous timeout (verified 1800s run). FIX: package-level TestMain shared pool + TruncateAll between tests (helper exists) → ~10x faster. | Medium | 4 | testutil | ++testing, ++db | DeepSeek V4 Flash | Medium | — |
